...
Sphere::Sphere(vec3 center, float radius, int slices, int stacks)
    : m_center(center), m_radius(radius), m_color(0, 0, 1, 1), m_spec_color(1, 1, 1, 1), m_vbo(NULL),
...
float Sphere::hitTime(vec3 o, vec3 d){
  d = o - d;
    float A = vec3::dotProduct(d,d);
    float B = vec3::dotProduct(2*d, (o-m_center));
    float C = vec3::dotProduct((o - m_center),(o - m_center)) - m_radius * m_radius;
    float diff = B * B - 4 * A * C;
    if (!(diff > 0))
    {
        return 0;
    } else {
        float t1 = - B + sqrt(diff);
        t1 = t1 / (2*A);
        float t2 = - B - sqrt(diff);
        t2 = t2 / (2*A);
        float res = fmin(t1,t2);
        return res;
    }
}
bool Sphere::isInside(vec3 point){
  if (fabs((point - m_center).length()) < m_radius+.1){
    // cout << "hit" << endl;
    return true;
  }
  return false;
